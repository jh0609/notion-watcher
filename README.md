...
상세 ready는 `DETAIL_READY_POLL_INTERVAL_MS`(기본 `250`) 간격으로 UUID 일치와 본문 20자 이상만 확인합니다. 카드 파싱이 완전하고 노출 옵션이 6개 미만인 상품은 최초 실행부터 카드만 사용합니다. 상세 조회는 카드 파싱이 불완전한 상품, 분석 `UNKNOWN` 상품, 이전 숨김 옵션 이력이 있는 상품, 이전 전체 옵션 수가 현재 노출 옵션 수보다 큰 상품, 노출 옵션이 6개 이상인 상품으로만 제한합니다. 자동 전체 상세 조회는 수행하지 않습니다.
